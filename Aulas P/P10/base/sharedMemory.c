...
int shmemCreate (int key, unsigned int size)
{
    ...
    // r-w user permissions..... create.... fail if 'key' block already exists
}
...
{
    void *add;     // temporary pointer                                                                               /* temporary pointer */

    add = shmat (shmid, (char *) NULL, 0); // shmaddr= NULL => endereco escolhido pelo sistema
    if (add != (void *) -1) {  // on error, (void *) -1 is returned
        *pAttAdd = (void *) add;
        return 0;
    }
    else return 1;
